...
Index = (function () {                                                                                               // 7
  ...
  function Index(config) {                                                                                           // 15
    ...
    if (!(config.engine instanceof Engine)) {                                                                        // 20
      throw new Meteor.Error('invalid-engine', 'engine needs to be instanceof Engine');                              // 21
                                                                                                                     ...
                                                                                                                     //
    config.name = (config.collection._name || '').toLowerCase();                                                     // 24
                                                                                                                     //
    this.config = _.assign(Index.defaultConfiguration, config);                                                      // 26
    ...
    config.engine.onIndexCreate(this.config);                                                                        // 30
  }                                                                                                                  //
                                                                                                                     //
  /**                                                                                                                //
  ...
  Index.prototype.search = (function () {                                                                            // 7
    function search(searchDefinition) {                                                                              // 53
      var options = arguments.length <= 1 || arguments[1] === undefined ? {} : arguments[1];                         //
                                                                                                                     //
      this.config.engine.checkSearchParam(searchDefinition, this.config);                                            // 54
                                                                                                                     //
      check(options, {                                                                                               // 56
        limit: Match.Optional(Number),                                                                               // 57
        skip: Match.Optional(Number),                                                                                // 58
        props: Match.Optional(Object)                                                                                // 59
      });                                                                                                            //
                                                                                                                     //
      options = {                                                                                                    // 62
        search: this._getSearchOptions(options),                                                                     // 63
        index: this.config                                                                                           // 64
      };                                                                                                             //
                                                                                                                     //
      if (!this.config.permission(options.search)) {                                                                 // 67
        throw new Meteor.Error('not-allowed', "You're not allowed to search this index!");                           // 68
      }                                                                                                              //
                                                                                                                     //
      return this.config.engine.search(searchDefinition, options);                                                   // 71
    }                                                                                                                //
                                                                                                                     //
    return search;                                                                                                   //
  })();                                                                                                              //
  ...
  Index.prototype._getSearchOptions = (function () {                                                                 // 7
    function _getSearchOptions(options) {                                                                            // 81
      return _.defaults(Meteor.userId ? { userId: Meteor.userId() } : {}, options, this.defaultSearchOptions);       // 82
    }                                                                                                                //
                                                                                                                     //
  ...
  babelHelpers.createClass(Index, null, [{                                                                           //
    ...
    get: function () {                                                                                               //
      return {                                                                                                       // 39
        permission: function () {                                                                                    // 40
          return true;                                                                                               //
        },                                                                                                           //
        defaultSearchOptions: function () {}                                                                         // 41
      };                                                                                                             //
    }                                                                                                                //
  }]);                                                                                                               //
  return Index;                                                                                                      //
