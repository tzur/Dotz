...
Meteor.methods({
  ...
  updateConnectionsMadeByUser(smartRef){
    ...
    Meteor.users.update({_id: smartRef.connection.connectedByUserId}, {$addToSet: {"connectionsMadeByUser": smartRef}})
  },

  updateLikesMadeByUser(smartRef){
    check(smartRef, Schema.dotSmartRef);
    Meteor.users.update({_id: Meteor.userId()}, {$addToSet: {"likesMadeByUser": smartRef}})
  },
  ...
  updateUserPeopleConnectedMyDotz(smartRef){
    ...
    if(smartRef.dot.ownerUserId != smartRef.connection.connectedByUserId){
      ...
      //});
      try {
        Meteor.users.update({_id: smartRef.dot.ownerUserId}, {$addToSet: {"peopleConnectedMyDotz": smartRef}});
      }
      catch(error){
        return error
      }
  ...
  updateUserPeopleLikedMyConnection(smartRef){
    ...
    if(Meteor.userId() != smartRef.connection.connectedByUserId && Meteor.userId() != smartRef.dot.ownerUserId ){
      ...
      //});
      try {
        Meteor.users.update({_id: smartRef.connection.connectedByUserId}, {$addToSet: {"peopleLikedMyConnection": smartRef}});
      }
      catch(error){
        return error
      }
  ...
  updateUserPeopleLikedMyDotz(smartRef){
    ...
    if(Meteor.userId() != smartRef.dot.ownerUserId && Meteor.userId() != smartRef.connection.connectedByUserId) {
      ...
      try {
        Meteor.users.update({_id: smartRef.dot.ownerUserId}, {$addToSet: {"peopleLikedMyDotz": smartRef}});
      }
      catch (error) {
        return error
      }
