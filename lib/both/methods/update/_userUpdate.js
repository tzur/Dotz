...
Meteor.methods({
  ...
  updateUserDotsAndTags(userDots, usertags){
    check(userDots, Array);
    check(usertags, Array);
    let updateOptions = {
      $set: {"profile.userDots": userDots, "profile.usertags": usertags}
    };
  ...
  updateLastDotzUserConnectedTo(dotId){
    check(dotId, String);
    let lastDotzUserConnectedTo = Meteor.user().profile.lastDotzUserConnectedTo;
    //console.log("lastDotzUserConnectedTo >>>>> " + lastDotzUserConnectedTo);

    if ( !lastDotzUserConnectedTo ) {
      console.log("Add .lastDotzUserConnectedTo to username: " +  Meteor.user().username)
      Meteor.user().profile.lastDotzUserConnectedTo = [];
    }
    //addToSet:
    let updateOptions = {
      $addToSet: {"profile.lastDotzUserConnectedTo": dotId}
    };
    _userUpdate(Meteor.userId(), updateOptions);

    //pop:
    if (lastDotzUserConnectedTo && lastDotzUserConnectedTo[8]) {
      let updateOptions_pop = {
        $pop: {"profile.lastDotzUserConnectedTo": -1}
      };
      _userUpdate(Meteor.userId(), updateOptions_pop);
    }
  },
