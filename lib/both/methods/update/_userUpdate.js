...
Meteor.methods({
  ...
  updateLastDotzConnectedByTheUser(smartRef){
    check(smartRef, Object);
    let lastDotzConnectedByTheUser = Meteor.user().profile.lastDotzConnectedByTheUser;
    console.log("lastDotzConnectedByTheUser >>>>> " + lastDotzConnectedByTheUser)

    //addToSet:
    let updateOptions = {
      $addToSet: {"profile.lastDotzConnectedByTheUser": smartRef}
    };
    _userUpdate(Meteor.userId(), updateOptions);

    //pop:
    if (lastDotzConnectedByTheUser && lastDotzConnectedByTheUser[8]) {
      let updateOptions_pop = {
        $pop: {"profile.lastDotzConnectedByTheUser": -1}
      };
