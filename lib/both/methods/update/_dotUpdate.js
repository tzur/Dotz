...
Meteor.methods({
  ...
  updateDotSuperTagsToFilterConnectedDotz(dotId, superTagsArray){
    check(dotId, String);
    check(superTagsArray, Array);

    let doc = {};
    doc.superTagsToFilterConnectedDotz = superTagsArray;
    let updateOptions = {
      $set: doc
    };
    _dotUpdate(dotId, updateOptions);
  ...
  //TODO >>>> fix it
  pushNewDotSuperTagsToFilterConnectedDotz(dotId, superTagsToFilter){
    check(dotId, String);
    check(superTagsToFilter, Array);

    superTagsToFilter.forEach(function (superTag) {
      Dotz.update({_id:dotId, "superTagsToFilterConnectedDotz.$._id": superTag._id}, {
            $addToSet: { "subTags": { $each: superTag.subTags } }
      });
      //console.log("*********** EACH *************")
    });
  ...
  updateRelatedDotzArray(profileDotId, relatedDotzArray){
    check(profileDotId, String);
    check(relatedDotzArray, Array);

    let doc = {};
    doc.relatedDotzArray = relatedDotzArray;
    let updateOptions = {
      $set: doc
    };
    _dotUpdate(profileDotId, updateOptions);
  },
