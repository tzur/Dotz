...
function isConcatWithEmptyString(node) {
    return node.operator === "+" && (node.left.type === "Literal" && node.left.value === "" || node.right.type === "Literal" && node.right.value === "");
}
    ...
    return {
        "UnaryExpression": function UnaryExpression(node) {
            ...
            if (options.boolean && isDoubleLogicalNegating(node)) {
                context.report(node, "use `Boolean({{code}})` instead.", { code: context.getSource(node.argument.argument) });
            }

            // ~foo.indexOf(bar)
            if (options.boolean && isBinaryNegatingOfIndexOf(node)) {
                context.report(node, "use `{{code}} !== -1` instead.", { code: context.getSource(node.argument) });
            }
        ...
        "BinaryExpression:exit": function BinaryExpressionExit(node) {
            ...
            if (nonNumericOperand) {
                context.report(node, "use `Number({{code}})` instead.", { code: context.getSource(nonNumericOperand) });
            }

            // "" + foo
            if (options.string && isConcatWithEmptyString(node)) {
                context.report(node, "use `String({{code}})` instead.", { code: context.getSource(getOtherOperand(node, "")) });
            }
        }
    };
};
...
//# sourceMappingURL=no-implicit-coercion-compiled.js.map