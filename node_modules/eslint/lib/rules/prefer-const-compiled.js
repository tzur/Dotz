...
module.exports = function (context) {
    ...
    function checkForVariables(scope) {
        ...
        for (var i = 0, end = variables.length; i < end; ++i) {
            ...
            if (statement && identifier && declaration.type === "VariableDeclaration" && declaration.kind === "let" && (statement.type !== "ForStatement" || statement.init !== declaration) && references.some(isInitializer) && references.every(isReadOnlyOrInitializer)) {
                context.report(identifier, "`{{name}}` is never modified, use `const` instead.", { name: identifier.name });
            }
        }
    }
...
//# sourceMappingURL=prefer-const-compiled.js.map